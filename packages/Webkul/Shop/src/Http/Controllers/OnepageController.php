...
class OnepageController extends Controller
{
    ...
    */
    ...
    public function get_max_bid(){
        
        $max_bid = DB::table('product_bids')
                      ->where('product_id',request()->product_id)
                      ->where('status','Pending')
                      ->max('bid_price');
        
        $pre_calculations = Cart::preCollectTotals();
              
        if(!empty($max_bid)){
            return response()->json(['max_bid'=>$max_bid,'pre_calculations'=>$pre_calculations],200);  
        }else{
            return response()->json(['max_bid'=>0,'pre_calculations'=>$pre_calculations],200);
        }  
    }
    
    public function saveBidPrice()
    {
       return response()->json([
            'charges' => Cart::saveBidPrice(request()->bid_price),
        ]); 
    }
    //saveBid
    public function saveBid()
    {
        ...
        DB::table('product_bids')->updateOrInsert([
            'product_id' => $cart->items[0]->product_id,
            'bid_price'  => $cart->items[0]->price,
            'customer_id' => $cart->customer_id,
            'product_price' => $cart->items[0]->price,
            'status'    => 'Pending',
            'cart_id'   => $cart->id
        ],[
            'cart_id' => $cart->id
