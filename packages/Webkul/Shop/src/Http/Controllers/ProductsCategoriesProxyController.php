...
class ProductsCategoriesProxyController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        if (preg_match('/^([\x{0621}-\x{064A}\x{4e00}-\x{9fa5}\x{3402}-\x{FA6D}\x{3041}-\x{30A0}\x{30A0}-\x{31FF}_a-z0-9-]+\/?)+$/u', $slugOrPath)) {
            ...
            if ($category = $this->categoryRepository->findByPath($slugOrPath)) {
                ...
                $products = $this->productRepository->getProductsRelatedToCategory($category->id);
                
                return view($this->_config['category_view'], compact('category','products'));
            }
    ...
    public function shopProducts(){
        //return request()->all();
        if(isset(request()->category_slug) && request()->category_slug == 'featured'){
            return view('shop::home.product-slider')->render();
        }
