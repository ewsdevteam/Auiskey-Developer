...
class Cart
{
     ...
     * @return bool
     */
    public function saveBidPrice($bidPrice)
    {
        if (! $cart = $this->getCart()) {
            return false;
        }
        ...
        if(isset($cart->items[0]->id)){
            ...
            DB::table('cart_items')->where('id',$cart->items[0]->id)->update([
                'price' => core()->convertPrice($bidPrice),
                'base_price' => $bidPrice,
                'total' => core()->convertPrice($bidPrice * 1),
                'base_total' => $bidPrice * 1,
            ]);
    ...
    public function preCollectTotals(){
        if (! $cart = $this->getCart()) {
            return false;
        }else{
           return $charges = [
